...
$.tablesorter.customPagerControls = function(options) {
	var defaults = {
		...
		link           : '<a href="javascript: void(0);">{page}</a>', // page element; use {page} to include the page number
	...
	$table.on('pagerInitialized pagerComplete', function (e, c) {
			...
			for(var i=c.filteredPages+1;i<=c.filteredPages+options.aroundCurrent+1;i++)
				pages.find('a[data-page='+i+']').hide();
			if(c.filteredPages!=c.totalPages)
				pages.find('a[data-page='+c.totalPages+']').hide();
			$table.find('.pagecount').html(pages.html());
			document.getElementById('panel'+$table.attr('id')).scrollIntoView();
		});
